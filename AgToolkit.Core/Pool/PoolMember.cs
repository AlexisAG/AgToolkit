...
namespace AgToolkit.AgToolkit.Core.Pool
{
	public class PoolMember : MonoBehaviour
	{
		...
		private void OnDisable()
		{
			...
			{
				if (ParentPool.poolData.autoSendBack)
				{
					SendBackToPool();
				}
			}
		...
		public void SendBackToPool()
		{
			BackToPool = true;
			Available = true;
			//wait one frame (otherwise causes error due to setactive(false) being called the same frame as reparenting)
			Invoke("DelayedRattached", 0.0001f);
		}
		...
		{
			Debug.Assert(DestroyedByPool, $"Pool Object {gameObject.name} destroyed outside of its pool {ParentPool.poolData.poolId}");
		}
	}
}